 ...
 *   - assemble performs the minimal roleBinding shape/consistency check and then
 *     enforces tools/writeScope within declared permissions (forbidden), positive
 *     budget & un-passed deadline (budget_exhausted), workspace snapshot revision ==
 *     canonical (stale_workspace_snapshot) and assembles a bounded TaskEnvelope
 *     (size cap TASK_ENVELOPE_MAX_SIZE_BYTES, sourceRefs only, NO transcript)
 *     while the bounded Bundle body goes FIRST into the vault;
 *   - stale_binding (binding version consistency) is NOT decided here — the
 *     frozen base line delegates it to Control.startRun, which rejects intent
 *     version inconsistency. assemble only does the minimal ref shape + subset
 *     consistency of RoleBindingRefV1 (no policy registry in P1-03);
 ...
 *
 * Lane C minimal-boundary notes:
 *   - "越权" (forbidden) = request.scope ⊄ request.declaredPermissions (tools
 *     and writeScope, independently) + roleBinding.policyRevision non-empty;
 *   - "旧绑定" (stale_binding) is NOT resolved here — Control.startRun rejects
 *     intent version inconsistency. assemble only does the minimal ref shape &
 *     consistency checks (the RoleBindingRefV1 shape is already validated by
 *     validateTaskContextRequest); it does not run a policy registry.
 *   - The envelope/sourceRefs are bounded: plan-revision + workspace refs only,
 *     NO artifact source and NO full transcript (the title + obligation
 *     SUMMARY lives in the vault bundle, never in the envelope).
 *   - Zero-write on every rejected / needs_material path EXCEPT the bundle
 *     body-first vault.put that precedes the envelope build (a later
 *     exceeds_size_cap leaves only an un-adopted Artifact).
 */
