...
import { recordArchitectureDecisionBriefFingerprint, recordArchitectureFindingFingerprint, recordArchitectureInspectionFingerprint, recordCandidateBaselineProposalFingerprint } from "../architecture-inspection.js";
import type {
...
import { ARCHITECTURE_BASELINE_FIXTURE_V1, architectureBaselinePinFor, buildInstallCommand } from "./governance-fixtures.js";
...
export function buildRecordArchitectureInspectionCommand(inspection: ArchitectureInspectionSnapshot, deps: BuildP112CommandDeps): RecordArchitectureInspectionCommand {
  return {
    ...
    schemaVersion: 1,
    identity: { projectId: inspection.intent.projectId ?? deps.projectId ?? P112_PROJECT, actor: deps.actor ?? { kind: "system", id: "architecture-reconciler" }, idempotencyKey: deps.idempotencyKey ?? deps.commandId + "-idem" },
    aggregateId: inspection.intent.inspectionId,
    expectedRevision: 0,
    correlationId: deps.correlationId ?? deps.commandId + "-corr",
    submittedAt: deps.submittedAt ?? P112_SCHEMA,
    payload: { inspection },
  };
}
...
export function buildRecordArchitectureFindingCommand(finding: ArchitectureFindingV1, deps: BuildP112CommandDeps): RecordArchitectureFindingCommand {
  return {
    ...
    schemaVersion: 1,
    identity: { projectId: finding.projectId, actor: deps.actor ?? { kind: "system", id: "architecture-reconciler" }, idempotencyKey: deps.idempotencyKey ?? deps.commandId + "-idem" },
...
export function buildRecordArchitectureDecisionBriefCommand(brief: ArchitectureDecisionBriefV1, deps: BuildP112CommandDeps): RecordArchitectureDecisionBriefCommand {
  return {
    ...
    schemaVersion: 1,
    identity: { projectId: brief.projectId, actor: deps.actor ?? { kind: "system", id: "architecture-reconciler" }, idempotencyKey: deps.idempotencyKey ?? deps.commandId + "-idem" },
...
export function buildRecordCandidateBaselineProposalCommand(proposal: ArchitectureCandidateProposalV1, deps: BuildP112CommandDeps): RecordCandidateBaselineProposalCommand {
  return {
    ...
    schemaVersion: 1,
    identity: { projectId: proposal.projectId, actor: deps.actor ?? { kind: "system", id: "architecture-reconciler" }, idempotencyKey: deps.idempotencyKey ?? deps.commandId + "-idem" },
