...
import { governanceContentDigest, governanceInstallFingerprint, governanceActivateFingerprint } from "../governance.js";
import type {
...
export function buildInstallCommand(
  ...
  const identity: CommandIdentity = {
    ...
    idempotencyKey: deps.idempotencyKey ?? "p1-02-governance-install",
  };
...
export function buildActivateCommand(
  ...
  const identity: CommandIdentity = {
    projectId: deps.projectId,
    actor: deps.actor ?? { kind: "human", id: "user-1" },
    idempotencyKey: deps.idempotencyKey ?? "p1-02-governance-activate",
  };
...
/** Expected CompletionPolicyRevisionRef for an install command (exact identity). */
