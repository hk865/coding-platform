...
/** Max materials authorized by a single grant (a bounded, auditable set). */
export const MATERIAL_ACCESS_MAX_MATERIALS = 64;
/** Purpose is an audit note, not an authority — keep it short. */
export const MATERIAL_ACCESS_PURPOSE_MAX_BYTES = 1024;
...
export type MaterialSourceSetV1 =
  | { kind: 'workspace_paths'; paths: string[] }
  | { kind: 'verification_workspace'; paths: ['.'] };
export type MaterialSourceScope = { projectId: string; workspaceId: string };
export type MaterialSourcePinV1 = MaterialSourceScope & {
  schemaVersion: 1;
  sourceSet: MaterialSourceSetV1;
  identity: { workspace: string; commit: string | null };
  manifestDigest: string;
};
export type MaterialSourceCaptureResult =
  | { status: 'sourced'; pin: MaterialSourcePinV1 }
  | { status: 'unavailable' | 'rejected' | 'stale'; issues: string[] };
...
export interface SourceApplicabilityPort {
  capture(query: MaterialSourceScope & { sourceSet: MaterialSourceSetV1 }, signal?: AbortSignal): Promise<MaterialSourceCaptureResult>;
}
...
export function validMaterialSourceSet(value: unknown): value is MaterialSourceSetV1 {
  ...
  if (set.kind === 'verification_workspace') return Array.isArray(set.paths) && set.paths.length === 1 && set.paths[0] === '.';
  if (set.kind !== 'workspace_paths' || !Array.isArray(set.paths) || set.paths.length < 1 || set.paths.length > 64) return false;
  if (!set.paths.every(path => typeof path === 'string' && path.length > 0 && path.length <= 1024 &&
    (path === '.' || (!/[\\:\0]/.test(path) && !path.split('/').some(part => !part || part === '.' || part === '..'))))) return false;
...
export type MaterialGrantIssuer =
  | ArtifactOwnerRunRef
  | { aggregateType: "Control"; projectId: string; goalId: string };
...
export type MaterialAccessGrantV1 = {
  ...
  history?: {
    ...
    crossWorkspace?: { sourceWorkspaceId: string; authorizedBy: ActorRef & { kind: 'human' } };
  };
};
...
/** Trusted host Control command; it does not authorize a new reader. */
export type RevokeMaterialAccessCommand = Omit<GrantMaterialAccessCommand,
  "commandType" | "expectedRevision" | "payload"> & {
  commandType: "RevokeMaterialAccess";
  expectedRevision: 1;
  payload: { grantRef: MaterialAccessGrantRef; reason: string };
};

export type RevokeMaterialAccessReceipt = GrantMaterialAccessReceipt;

export type MaterialAccessRevokedEvent = Omit<MaterialAccessGrantedEvent,
  "eventType" | "aggregateRevision" | "payload"> & {
  eventType: "MaterialAccessRevoked";
  aggregateRevision: 2;
  payload: { grant: MaterialAccessGrantV1; revocation: MaterialAccessRevocationV1 };
};

export function revokeMaterialAccessFingerprint(command: RevokeMaterialAccessCommand): CommandFingerprint {
  return sha256Hex(canonicalJson({ schemaVersion: command.schemaVersion, commandType: command.commandType,
    projectId: command.identity.projectId, aggregateId: command.aggregateId,
    expectedRevision: command.expectedRevision, payload: command.payload } as never)) as CommandFingerprint;
}

export function materialAccessGrantRefFor(
  projectId: string,
  workspaceId: string,
  goalId: string,
  grantId: string,
): MaterialAccessGrantRef {
  return { aggregateType: "MaterialAccessGrant", projectId, workspaceId, goalId, grantId };
}

/** Deterministic grant id: one grant per (reader, material set, basis). */
export function materialAccessGrantIdFor(reader: ArtifactOwnerRunRef, materials: ArtifactRef[], basis: MaterialBasisV1): string {
  return "grant-" + sha256Hex(canonicalJson({ reader, materials, basis } as never)).slice(0, 32);
}
...
export function sameArtifactOwnerRunRef(a: ArtifactOwnerRunRef | null, b: ArtifactOwnerRunRef | null): boolean {
  ...
  if (a.aggregateType !== b.aggregateType) return false;
...
export type GrantMaterialAccessCommand = {
  commandId: string;
  commandType: "GrantMaterialAccess";
  schemaVersion: 1;
  identity: CommandIdentity;
  /** grantId — the immutable grant aggregate (created exactly once). */
  aggregateId: string;
  expectedRevision: 0;
  correlationId: string;
  submittedAt: string;
  payload: { grant: MaterialAccessGrantV1 };
};

type GrantMaterialAccessRejectionCode =
  | "invalid"
  | "not_found"
  | "scope_mismatch"
  | "revision_conflict"
...
export type GrantMaterialAccessReceipt =
  | {
      status: "committed";
      commandId: string;
      replayed: boolean;
  ...
  | { status: "rejected"; commandId: string; code: GrantMaterialAccessRejectionCode; issues?: string[] };
...
export type MaterialAccessGrantedEvent = {
  ...
  actor: ActorRef;
  occurredAt: string;
  payload: { grant: MaterialAccessGrantV1; grantedAt: string };
};

export function grantMaterialAccessFingerprint(command: GrantMaterialAccessCommand): CommandFingerprint {
  const shape = {
    ...
    payload: { grant: command.payload.grant },
  };
...
/** Exact candidate lookup for authorization; never subject to display limits. */
export type MaterialAccessGrantLookup = { reader: ArtifactOwnerRunRef; material: ArtifactRef };

export function matchesMaterialAccessLookup(grant: MaterialAccessGrantV1, query: MaterialAccessGrantLookup): boolean {
  return grant.scope.projectId === query.reader.projectId &&
    sameArtifactOwnerRunRef(grant.reader, query.reader) && grant.materials.some(ref => sameArtifactRef(ref, query.material));
}
...
// ------------------------------------------------------------------------ //
// Read-model display view (authorization uses the separate exact lookup)   //
// ------------------------------------------------------------------------ //

export type MaterialAccessGrantViewQuery = {
  projectId: string;
  workspaceId?: string;
  goalId?: string;
  /** Only grants that include a material with this body digest. */
  materialDigest?: string;
  /** Only grants issued to this reader run id. */
  readerRunId?: string;
  limit?: number;
};

export type MaterialAccessGrantRow = {
  ref: MaterialAccessGrantRef;
  revision: number;
  grant: MaterialAccessGrantV1;
...
export type MaterialAccessGrantViewResult =
  | { status: "ready"; grants: MaterialAccessGrantRow[]; sourceCursor: CommitCursor }
  | { status: "not_ready"; observedCursor: CommitCursor | null };

/** Bounded display limit. Permission resolution must use the exact lookup. */
export const MATERIAL_ACCESS_VIEW_MAX_ROWS = 256;
