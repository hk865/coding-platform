/** dispatch protocol schema validation. Structural checks do not grant authority. */
import { canonicalJson } from '../fingerprint.js';
import { RUNTIME_EVENT_TYPES } from '../dispatch.js';
import { TASK_ENVELOPE_MAX_SIZE_BYTES } from '../task-envelope.js';
import type { ValidationIssue } from './common.js';
import { isRecord, stringField, numberField, stringArrayField } from './common.js';
...
export function validateDeclaredPermissions(value: unknown, path: string, issues: ValidationIssue[]): void {
  if (!isRecord(value)) {
    issues.push({ path, code: "bad_type", message: path + " must be an object" });
    return;
  }
...
export function validateRunRef(value: unknown, path: string, issues: ValidationIssue[]): void {
  if (!isRecord(value)) {
    issues.push({ path, code: "bad_type", message: path + " must be an object" });
    return;
  }
...
function validateContextManifest(value: unknown): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  if (!isRecord(value)) {
...
function validateRuntimeEvent(value: unknown): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  if (!isRecord(value)) {
