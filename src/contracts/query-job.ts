...
export const QUERY_JOB_MAX_ROUNDS = 4;
export const QUERY_JOB_ANSWER_MAX_BYTES = 16 * 1024;
export const QUERY_JOB_MAX_SOURCES = 64;
...
export function queryRunRefFor(projectId: string, workspaceId: string, queryJobId: string, runId: string): QueryRunRef {
  return { aggregateType: "QueryRun", projectId, workspaceId, queryJobId, runId };
}
export function queryJobAnswerRefFor(projectId: string, workspaceId: string, queryJobId: string, answerId: string): QueryJobAnswerRef {
  return { aggregateType: "QueryJobAnswer", projectId, workspaceId, queryJobId, answerId };
}
...
export type QueryJobV1 = {
  ...
  closeReason: { code: "timeout" | "gap" | "failed" | "stale_source" | "cancelled"; message: string } | null;
  submittedAt: string;
  updatedAt: string;
};
...
export type QueryJobAnswerV1 = {
  ...
  /** A follow-up answer that references a prior answer carries the prior ref. */
...
export type QueryJobAnswerSnapshot = { ref: QueryJobAnswerRef; revision: 1; schemaVersion: 1; answer: QueryJobAnswerV1 };

// ------------------------------------------------------------------------ //
// Commands / receipts                                                       //
// ------------------------------------------------------------------------ //

export type SubmitQueryJobCommand = {
  commandId: string;
  commandType: "SubmitQueryJob";
  schemaVersion: 1;
...
export type RecordQueryAnswerCommand = {
  commandId: string;
  commandType: "RecordQueryAnswer";
  schemaVersion: 1;
  identity: CommandIdentity;
  aggregateId: string;
...
export type CloseQueryJobCommand = {
  commandId: string;
  commandType: "CloseQueryJob";
  schemaVersion: 1;
  identity: CommandIdentity;
  aggregateId: string;
  expectedRevision: number;
...
export type QueryJobSubmittedEvent = {
  eventId: string; eventType: "QueryJobSubmitted"; schemaVersion: 1;
  projectId: string; workspaceId: string; aggregateType: "QueryJob"; aggregateId: string; aggregateRevision: 1;
  causationId: string; correlationId: string; idempotencyKey: string; actor: ActorRef; occurredAt: string;
  payload: { job: QueryJobV1 };
};
...
export type QueryJobAnswerRecordedEvent = {
  eventId: string; eventType: "QueryJobAnswerRecorded"; schemaVersion: 1;
  projectId: string; workspaceId: string; aggregateType: "QueryJob"; aggregateId: string; aggregateRevision: number;
  causationId: string; correlationId: string; idempotencyKey: string; actor: ActorRef; occurredAt: string;
  payload: { answer: QueryJobAnswerV1; job: QueryJobV1; run: QueryRunV1 };
};

export type QueryJobClosedEvent = {
  eventId: string; eventType: "QueryJobClosed"; schemaVersion: 1;
  projectId: string; workspaceId: string; aggregateType: "QueryJob"; aggregateId: string; aggregateRevision: number;
  causationId: string; correlationId: string; idempotencyKey: string; actor: ActorRef; occurredAt: string;
  payload: { reason: { code: "timeout" | "gap" | "failed" | "stale_source" | "cancelled"; message: string }; job: QueryJobV1; run: QueryRunV1 };
};
...
export function submitQueryJobFingerprint(command: SubmitQueryJobCommand): CommandFingerprint {
  return sha256Hex(canonicalJson({ schemaVersion: 1, commandType: command.commandType, projectId: command.identity.projectId, aggregateId: command.aggregateId, expectedRevision: command.expectedRevision, payload: { intent: command.payload.intent, runId: command.payload.runId } })) as CommandFingerprint;
}
export function recordQueryAnswerFingerprint(command: RecordQueryAnswerCommand): CommandFingerprint {
...
// ------------------------------------------------------------------------ //
// Query context port (ContextCompiler)                                      //
// ------------------------------------------------------------------------ //

export type QueryContextRequestV1 = {
  schemaVersion: 1;
  requestId: string;
  queryJobRef: QueryJobRef;
  runRef: QueryRunRef;
...
export type QueryContextResultV1 =
  | { status: "ready"; bundleRef: ArtifactRef; manifest: { queryJobRef: QueryJobRef; selectedSources: { kind: string; refKey: string; version: string | null }[]; truncated: string[]; totalBytes: number; freshnessCursor: CommitCursor | null; noTranscript: true } }
  | { status: "needs_material"; gaps: { code: "focus_not_found" | "vault_unavailable" | "budget_exhausted" | "stale_versions"; message: string }[] }
  | { status: "rejected"; code: "invalid_request" | "forbidden_tool_or_scope" | "work_not_found" | "unavailable"; message: string };

export interface QueryContextPort {
  assembleQueryContext(request: QueryContextRequestV1): Promise<QueryContextResultV1>;
}

// ------------------------------------------------------------------------ //
// Read-only query port (WorkerRuntime)                                      //
// ------------------------------------------------------------------------ //

export type ReadOnlyQueryResultV1 = {
  schemaVersion: 1;
  runRef: QueryRunRef;
...
// ------------------------------------------------------------------------ //
// Snapshot / PublicSnapshot ports (DispatchEngine / WorkerRuntime)          //
// ------------------------------------------------------------------------ //

export type PublicSnapshotQueryV1 = { schemaVersion: 1; projectId: string; workspaceId: string; runRef: RunRef | null };
export type PublicSnapshotResultV1 =
  | { status: "report"; report: { runRef: RunRef; summary: string; lastEventSeq: number | null; terminalOutcome: string | null; reportRef: ArtifactRef | null; at: string; noHiddenContextRead: true } }
...
// ------------------------------------------------------------------------ //
// QueryJob drive port (DispatchEngine)                                      //
// ------------------------------------------------------------------------ //

export type QueryJobDriveTrigger = { reason: string; maxIntents?: number };
export type QueryJobDriveResult = { scanned: number; started: number; answered: number; closed: number; failures: { intentId: string; code: string; message: string }[] };

export interface QueryJobDrivePort {
  driveQuery(trigger: QueryJobDriveTrigger): Promise<QueryJobDriveResult>;
}
