...
import {
  validateAcquireWorkspaceReadLeaseCommand,
  validateAcquireWorkspaceWriteLeaseCommand,
  validateReleaseWorkspaceLeaseCommand,
} from "../contracts/validation.js";
import { evaluateWorkspaceOperation } from "../contracts/workspace-capability.js";
...
} from "../contracts/fixtures/workspace-fixtures.js";
  ...
  async acquireWriteLease(command: AcquireWorkspaceWriteLeaseCommand): Promise<AcquireWriteLeaseReceipt> {
    ...
    if (receipt.status === "committed") {
      return {
        ...
        commandId,
        replayed: receipt.replayed,
        leaseRef: workspaceWriteLeaseRefFor(projectId, command.aggregateId),
        eventIds: receipt.eventIds,
        commitCursor: receipt.commitCursor,
      };
    }
    return this.rejectWrite(commandId, mapCommitRejection(receipt.code));
  }
