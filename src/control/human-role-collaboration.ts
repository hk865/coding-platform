...
} from "../contracts/fixtures/human-role-collaboration-fixtures.js";
...
async function installPolicyImpl(
  ...
  const expectedDigest = coordinationPolicyContentDigest(payload.content, payload.policyId, 1);
...
function validateInstallShape(command: unknown): string[] {
  ...
  if (!isRecord(allowed) || allowed["inScopeRework"] !== true || allowed["inScopeTesting"] !== true) issues.push("payload.content.allowed");
