...
import { resolveProjectArchitectureBaseline, loadProjectArchitectureBaselineActive } from "../../data/state-ledger/governance-records.js";
...
async function recordDecisionImpl(
  ...
  if (!refMatch || !digestMatch || !fromMatch ||
    candidate.projectId !== decision.projectId || candidate.workspaceId !== decision.workspaceId ||
    !canonicalJsonEqual(decision.subject.fromPin, candidate.parentSourcePin)) {
...
async function recordGateImpl(
  ...
  const candidate = (candidateResult.snapshot as { candidate: CandidateArchitectureBaselineV1 }).candidate;
  if (candidate.projectId !== gate.projectId || candidate.workspaceId !== gate.workspaceId) {
    return { status: "rejected", commandId: command.commandId, code: "invalid", issues: ["candidate scope mismatch"] };
  }
...
async function recordActivationImpl(
  ...
  const activationRef = baselineActivationRefFor(activation.projectId, activation.workspaceId, activation.activationId);
  ...
  // A PASS belongs only to the exact candidate and scope authorized by the decision.
  if (
    !canonicalJsonEqual(gate.candidateRef, decision.subject.candidateRef) ||
    decision.projectId !== activation.projectId || decision.workspaceId !== activation.workspaceId ||
    gate.projectId !== activation.projectId || gate.workspaceId !== activation.workspaceId
  ) {
    return { status: "rejected", commandId: command.commandId, code: "target_mismatch" };
  }
  ...
  const candidate = (candidateResult.snapshot as { candidate: CandidateArchitectureBaselineV1 }).candidate;
  if (!canonicalJsonEqual(candidate.proposalRef, activation.proposalRef) ||
    !canonicalJsonEqual(candidate.parentSourcePin, activation.fromPin) ||
    !canonicalJsonEqual(decision.subject.fromPin, activation.fromPin) ||
    candidate.contentDigest !== activation.toPin.digest ||
    activation.toPin.ref.projectId !== activation.projectId || activation.fromPin.ref.projectId !== activation.projectId) {
    return { status: "rejected", commandId: command.commandId, code: "target_mismatch" };
  }
  ...
  // Capture the active-ref version before resolving its immutable content. A
  // concurrent default move either fails the pin check or the atomic CAS below.
  ...
  if (!proposalSourceMatch || !activeMatch || activeVersion.status !== "found") {
    return { status: "rejected", commandId: command.commandId, code: "source_stale" };
  }

  const eventId = deps.eventId();
  const occurredAt = deps.now();
  const workspace = await deps.ledger.load({ aggregateType: "Workspace", projectId: activation.projectId, workspaceId: activation.workspaceId });
...
function validateMaterializeShape(command: unknown): string[] {
  ...
  if (isRecord(ref) && isRecord(command["identity"]) && ref["projectId"] !== command["identity"]["projectId"]) issues.push("proposalRef.projectId");
  return issues;
}

function validateRecordIdentity(command: Record<string, unknown>, entity: Record<string, unknown>, idKey: string): string[] {
  const identity = command["identity"];
  if (!isRecord(identity) || !isRecord(identity["actor"]) || !isString(identity["actor"]["kind"]) || !isString(identity["actor"]["id"]) || !isString(identity["idempotencyKey"])) return ["identity"];
  if (identity["projectId"] !== entity["projectId"] || command["aggregateId"] !== entity[idKey] || command["expectedRevision"] !== 0) return ["identity mismatch"];
  return [];
}
...
function validateDecisionShape(command: unknown): string[] {
  ...
  issues.push(...validateRecordIdentity(command, d, "decisionId"));
...
function validateGateShape(command: unknown): string[] {
  ...
  issues.push(...validateRecordIdentity(command, g, "gateId"));
...
function validateActivationShape(command: unknown): string[] {
  ...
  issues.push(...validateRecordIdentity(command, a, "activationId"));
