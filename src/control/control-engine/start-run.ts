...
import { validateDispatchStartCommand } from '../../contracts/validation/dispatch.js';
import { buildDispatchStartLedgerCommit } from "./records/dispatch.js";
...
import { reviewMaterialReadVersions } from './reviewer-work.js';
...
async function startRunImpl(
  ...
  // Review identity must exist canonically. Ordinary envelopes cannot invent it.
  if (!run.work && (envelope.work || envelope.reviewInput || attempt.work || intent.work)) return { status: 'rejected', commandId: command.commandId, code: 'invalid' };
  let reviewWork: ReviewWorkSnapshot | null = null;
  const reviewReadVersions: import('../../contracts/ledger.js').ExpectedVersion[] = [];
  if (run.work) {
    const loaded = await deps.ledger.load(run.work.reviewWorkRef);
    if (loaded.status !== 'found' || loaded.snapshot.ref.aggregateType !== 'ReviewWork') return { status: 'rejected', commandId: command.commandId, code: 'invalid' };
    reviewWork = loaded.snapshot as ReviewWorkSnapshot;
    const same = (a: unknown, b: unknown) => a === undefined || b === undefined ? a === b : canonicalJson(a as never) === canonicalJson(b as never);
    const input = envelope.reviewInput;
    if (!input || !same(envelope.work, run.work) || !same(attempt.work, run.work) || !same(intent.work, run.work) || !same(reviewWork.reviewerRunRef, run.ref) || !same(reviewWork.reviewerAttemptRef, attempt.ref) || !same(reviewWork.roleBinding, envelope.roleBinding) || !same(envelope.permissions.tools, ['read']) || envelope.permissions.writeScope.length || !same(intent.declaredPermissions, { tools: ['read'], writeScope: [] }) || input.descriptorDigest !== reviewWork.descriptorRef.digest || input.packetDigest !== input.packetRef.digest || !input.inputDigest || !input.grantRefs.length || (reviewWork.input && !same(reviewWork.input, input))) return { status: 'rejected', commandId: command.commandId, code: 'invalid' };
    for (const ref of input.grantRefs) {
      const result = await deps.ledger.load(ref);
      if (result.status !== 'found') return { status: 'rejected', commandId: command.commandId, code: 'invalid' };
      const grant = result.snapshot as MaterialAccessGrantSnapshot;
      if (grant.revocation || grant.grant.history || !same(grant.grant.reader, run.ref) || !same(grant.grant.basis.planRef, run.planRef) || grant.grant.basis.workspaceRevision !== run.workspaceSnapshot.revision || grant.grant.basis.sourceDigest !== reviewWork.descriptor.materialIdentity.sourceDigest) return { status: 'rejected', commandId: command.commandId, code: 'stale_binding' };
      reviewReadVersions.push({ ref, revision: grant.revision });
    }
    try { reviewReadVersions.push(...await reviewMaterialReadVersions(deps.ledger, reviewWork)); }
    catch { return { status: 'rejected', commandId: command.commandId, code: 'stale_binding' }; }
  }
  ...
  let commit: import('../../contracts/ledger.js').LedgerCommit = batch;
  if (reviewWork) {
    const next: ReviewWorkSnapshot = { ...reviewWork, revision: 2, input: envelope.reviewInput! };
    const reviewBatch: ReviewLedgerCommitV1 = { ...batch, commitKind: 'review-start', snapshots: [...batch.snapshots, next], expectedVersions: [...batch.expectedVersions, { ref: reviewWork.ref, revision: 1 }, ...reviewReadVersions], events: [...batch.events, { eventId: deps.eventId(), eventType: 'ReviewInputBound', aggregateType: 'ReviewWork', schemaVersion: 1, projectId, workspaceId, aggregateId: reviewWork.ref.reviewId, aggregateRevision: 2, causationId: command.commandId, correlationId: command.correlationId, idempotencyKey: command.identity.idempotencyKey, actor: command.identity.actor, occurredAt, payload: { work: next } }] };
    commit = reviewBatch;
  }
  const receipt = await deps.ledger.commit(commit);
