...
import { reviewReductionIsCurrent } from './reviewer-work.js';
...
export async function loadLivePlan(
  ...
  for (const task of plan.tasks) {
    ...
    if (
      ...
      let livePhase = taskReductionPhaseToPlanPhase(
        (reductionResult.snapshot as TaskReductionSnapshot).phase,
      );
      if (livePhase === 'satisfied' && !await reviewReductionIsCurrent(ledger, plan, reductionResult.snapshot as TaskReductionSnapshot)) livePhase = 'blocked';
