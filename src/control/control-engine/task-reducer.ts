...
import { reduceTaskVerification } from "./policies/task-reduction.js";
import { type TaskReductionInput } from "../../contracts/reduction.js";
import { validateReduceTaskCommand } from '../../contracts/validation/evidence.js';
import { buildTaskReductionLedgerCommit, buildTaskReductionSnapshot } from "./records/evidence.js";
import type { ControlEngineDeps } from "./control-engine.js";
import { taskEvidenceIndexRefFor } from "../../contracts/evidence.js";
import { qualifyReviewEvidence, reviewerCoverage, withReviewReadVersions } from './reviewer-work.js';
...
async function reduceTaskImpl(
  ...
  evidence = await qualifyReviewEvidence(deps.ledger, plan, { projectId, goalId, taskId }, evidence);
  ...
  // A reviewer-bearing legacy task also guards protocol absence: adoption must
  // invalidate an in-flight computation made under legacy PASS semantics.
  if (plan.reviewAdmissionProtocol || reviewerCoverage(plan, taskId, false).length || reviewVersions.some(v => v.ref.aggregateType === 'TaskReviewProtocol' && v.revision > 0)) {
    batch.reviewProtocol = 'independent-review-v1';
    batch.expectedVersions.push(...reviewVersions);
  }
