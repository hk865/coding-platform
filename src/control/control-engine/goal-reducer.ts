...
import { qualifyReviewEvidence, reviewReductionIsCurrent } from './reviewer-work.js';
import type {
  ReduceGoalCommand,
  ReduceGoalReceipt,
  GoalReductionInput,
...
import { goalPhaseRefFor } from "../../contracts/goal-phase.js";
import { reduceGoalPhase, renderGoalCompletionExplanation, reconcileGoalSideEffects } from "./policies/goal-phase.js";
import { validateReduceGoalCommand } from '../../contracts/validation/evidence.js';
import { buildGoalPhaseSnapshot, buildGoalReductionLedgerCommit } from "./records/goal-phase.js";
import { taskReductionRefFor } from "../../contracts/reduction.js";
import { runRefFor, taskLeaseRefFor } from "../../contracts/dispatch.js";
import { taskEvidenceIndexRefFor, evidenceRefFor, requirementKeyOf } from "../../contracts/evidence.js";
...
import { withReviewReadVersions } from './reviewer-work.js';
...
async function reduceGoalImpl(
  ...
  const reviewVersions: import('../../contracts/ledger.js').ExpectedVersion[] = [];
  deps = { ...deps, ledger: withReviewReadVersions(deps.ledger, reviewVersions) };
  ...
  return commitGoalReduction(deps, command, {
    ...
    ...(plan && (plan.reviewAdmissionProtocol || plan.obligations.some(o => o.verificationRequirements.some(v => v.kind === 'reviewer'))) ? { reviewReadVersions: reviewVersions } : {}),
  });
}
...
export async function buildGoalReductionInput(
  ...
  for (const task of plan.tasks) {
    ...
    const qualified = await qualifyReviewEvidence(deps.ledger, plan, { projectId, goalId, taskId: task.taskId }, evidence);
    const effectiveSet = selectEffectiveEvidenceSet(qualified, plan, currentAnchor);
    ...
    taskReductions.push({
      ...
      reductionPhase: reduction?.phase === 'satisfied' && !await reviewReductionIsCurrent(deps.ledger, plan, reduction) ? 'blocked' : reduction?.phase ?? null,
...
export async function commitGoalReduction(
  ...
  deps2: {
    ...
    reviewReadVersions?: import('../../contracts/ledger.js').ExpectedVersion[];
  },
): Promise<ReduceGoalReceipt> {
  ...
  if (deps2.reviewReadVersions?.length) {
    batch.reviewProtocol = 'independent-review-v1';
    batch.expectedVersions.push(...deps2.reviewReadVersions.filter(v => canonicalJson(v.ref) !== canonicalJson(snapshot.ref)));
  }
