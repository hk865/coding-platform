...
`SELECT MAX(seq) AS max_seq FROM goal_timeline WHERE project_id = ? AND goal_id = ?`
  ...
  private p112AppendRow(table: "architecture_brief_rows" | "architecture_proposal_rows", key: string, entry: unknown, cursor: CommitCursor): void {
  ...
  private p108ApplyPlanRevisionAccepted(event: PlanRevisionAcceptedEvent, cursor: CommitCursor): void {
    ...
    this.p108AppendTimeline(event, cursor, "plan_accepted", { goalId },
