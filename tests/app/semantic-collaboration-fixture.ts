...
export async function semanticCollaborationFixture(createApplication:ApplicationFactory, onCompleted?: (result:{base:string;scope:{projectId:string;workspaceId:string;goalId:string};final:any;originalRunId:string})=>Promise<void>) {
  ...
  const client:ModelClientPort={async *stream(request){
    ...
    const kind=input.includes('"execution_coordination"')||input.includes('\\"execution_coordination\\"')?'feedback':request.tools.some(t=>t.name==='edit')?(input.includes('已提交的失败事实')?'rework':'worker'):'planning';
    ...
    const call=kind==='planning'&&n===0?{name:'read',arguments:{path:'normalize.py'}}:
      kind==='feedback'&&n===0?{name:'read',arguments:{path:'RULES.md'}}:
    ...
    const result=kind==='planning'?plan:kind==='worker'?{kind:'execution_feedback',category:'missing_material',summary:'The implementation does not specify whitespace and case semantics.',question:'Investigate RULES.md and supply the exact label normalization rule and its source for this work.'}:
  ...
  try{
    ...
    await post('/api/goals',{...scope,requestId:scope.goalId,objective:'Implement label normalization according to RULES.md and verify edge cases.'});
    ...
    const final=await until(s=>s.liveRuns.some((r:any)=>r.spec.taskId.startsWith('rework-')&&r.rounds?.some((round:any)=>round.outcome==='PASS' && round.status==='completed' && round.control.goalPhase && round.aggregate?.admissions.every((a:any)=>a.status==='admitted')) && s.matrix.matrix.rows.some((row:any)=>row.taskId===r.spec.taskId && row.livePhase==='satisfied')));
    ...
    assert.equal(final.matrix.matrix.rows.find((row:any)=>row.taskId===successor.spec.taskId).livePhase,'satisfied');
    ...
    const before=requests.length;await app.close();app=await createApplication(join(dir,'data'),options);await listen();
    ...
    await onCompleted?.({base,scope,final,originalRunId:run.spec.runId});
