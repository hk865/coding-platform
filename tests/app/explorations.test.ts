...
it('executes a real read-only dependency chain, requires operator review and preserves reports across restart', async () => {
  ...
  grantsDb.close();
  const grants = grantRows.map(row => JSON.parse(row.entry_json) as { grant: { reader: { runId: string }; issuedBy: { aggregateType: string }; basis: { workspaceRevision: number | null; sourceDigest: string; sourcePin?: { manifestDigest: string; sourceSet: { paths: string[] } } }; materials: unknown[] } });
  ...
  expect(grants.every(g => g.grant.basis.sourcePin?.manifestDigest === g.grant.basis.sourceDigest && g.grant.basis.sourcePin.sourceSet.paths.join() === '.')).toBe(true);
