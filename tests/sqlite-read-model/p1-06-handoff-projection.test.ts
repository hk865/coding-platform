 ...
 *
 * Mirror of tests/read-model/p1-06-handoff-projection.test.ts against the
 * SQLite adapter (dual-adapter requirement). Covers:
 *   - incremental projection (ordered timeline);
 ...
 *   - freshness (not_ready != not_found; covered-atLeastCursor + no row ->
 *     not_found; no atLeastCursor + no row -> not_ready);
 *   - outcomeUnknownPreserved constant true;
 *   - projection persists across close()/reopen (persistence of the projection
 *     row + checkpoint) and replays to the same view;
 *   - handled event types (HandoffRecorded / ReplacementClaimed) advance
 *     WITHOUT stalling.
 */
...
function evSet(projectId: string) {
  return {
    rec: recordedEvent(projectId, "packet-1", "ev-rec-1"),
    claim: claimedEvent(projectId, "packet-1", "ev-claim-2", "att-b-2", "run-b-2"),
    evA: evidenceEvent(projectId, "ev-a-3", "ev-ev-3", "run-a-1"),
    evB: evidenceEvent(projectId, "ev-b-4", "ev-ev-4", "run-b-2"),
  };
}
...
describe("P1-06 SQLite handoff-provenance projection", () => {
  ...
  it("rebuild equivalence: empty fresh instance -> full replay == incremental (field-for-field)", async () => {
    const projectId = "proj-alpha";
    const { rec, claim, evA, evB } = evSet(projectId);
  ...
  it("projection persists across close()/reopen and the reopen replays to the same view", async () => {
    const projectId = "proj-alpha";
    ...
    try {
      ...
      await rm.advance(page([pos(evA, 3)], 3));
      await rm.advance(page([pos(evB, 4)], 4));
      const before = await rm.handoffProvenance({ projectId, goalId: P106_GOAL, taskId: P106_TASK_ID });
      expect(before.status).toBe("ready");
      const beforeJson = before.status === "ready" ? json(before.provenance) : "";
      await rm.close();
      ...
      if (rebuiltRes.status === "ready") expect(json(rebuiltRes.provenance)).toBe(beforeJson);
      await rebuilt.close();
    } finally {
