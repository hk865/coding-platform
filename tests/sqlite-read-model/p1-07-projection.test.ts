 ...
 *
 * Mirror of tests/read-model/p1-07-projection.test.ts against the SQLite
 * adapter (dual-adapter requirement). Covers:
 *   - 6-event projection into the 3 views (field-for-field);
 *   - rebuild equivalence (empty fresh instance -> full replay == incremental);
 *   - full-key isolation (two Projects / two workspaces never collide);
 *   - freshness (not_ready != not_found);
 *   - persistence across close()/reopen (projection row + checkpoint survive);
 *   - handled event types advance WITHOUT stalling.
 */
...
function readGranted(projectId: string, workspaceId: string, leaseId: string, runId: string, eventId: string): import("../../src/contracts/workspace-lease.js").WorkspaceReadLeaseGrantedEvent {
  const command = buildP107AcquireReadLeaseCommand({ commandId: "cmd-" + eventId, projectId, leaseId, workspaceId, scope: P107_SCOPE_READER_A, holder: { runRef: runRef(projectId, runId), attemptRef: attRef(projectId, P107_TASK_READER_A, "att-" + runId), roleBinding: P107_ROLE_BINDING_READER_V1 } });
  return buildWorkspaceReadLeaseAcquireLedgerCommit(command, { eventId, occurredAt: P107_SCHEMA, indexSnapshot: { ref: workspaceReadLeaseIndexRefFor(projectId, workspaceId), revision: 0, schemaVersion: 1, activeReadLeases: [] } }).events[0];
}
function readReleased(projectId: string, workspaceId: string, leaseId: string, runId: string, eventId: string): import("../../src/contracts/workspace-lease.js").WorkspaceReadLeaseReleasedEvent {
  ...
  const active: import("../../src/contracts/workspace-lease.js").WorkspaceReadLeaseSnapshot = { ref: workspaceReadLeaseRefFor(projectId, leaseId), revision: 1, schemaVersion: 1, lease: { schemaVersion: 1, leaseId, projectId, workspaceId, scope: P107_SCOPE_READER_A, holder: { runRef: runRef(projectId, runId), attemptRef: attRef(projectId, P107_TASK_READER_A, "att-" + runId), roleBinding: P107_ROLE_BINDING_READER_V1 }, grantedAt: P107_SCHEMA, expiresAt: null, status: "active", releasedAt: null, releasedBy: null } };
  return buildWorkspaceReadLeaseReleaseLedgerCommit(command, active, { eventId, occurredAt: P107_SCHEMA, indexSnapshot: { ref: workspaceReadLeaseIndexRefFor(projectId, workspaceId), revision: 1, schemaVersion: 1, activeReadLeases: [{ leaseId, scope: P107_SCOPE_READER_A }] } }).events[0];
}
function writeGranted(projectId: string, workspaceId: string, leaseId: string, runId: string, scope: import("../../src/contracts/workspace-lease.js").ConflictScopeV1, eventId: string): import("../../src/contracts/workspace-lease.js").WorkspaceWriteLeaseGrantedEvent {
  const command = buildP107AcquireWriteLeaseCommand({ commandId: "cmd-" + eventId, projectId, leaseId, workspaceId, scope, declaredWriteScope: [scope.id], holder: { runRef: runRef(projectId, runId), attemptRef: attRef(projectId, P107_TASK_WRITER, "att-" + runId), roleBinding: P107_ROLE_BINDING_WRITER_V1 } });
  return buildWorkspaceWriteLeaseAcquireLedgerCommit(command, { eventId, occurredAt: P107_SCHEMA, indexSnapshot: { ref: workspaceWriteLeaseIndexRefFor(projectId, workspaceId), revision: 0, schemaVersion: 1, activeLeaseId: null, activeScope: null, holderRunRef: null } }).events[0];
}
function writeReleased(projectId: string, workspaceId: string, leaseId: string, runId: string, eventId: string): import("../../src/contracts/workspace-lease.js").WorkspaceWriteLeaseReleasedEvent {
  ...
  const active: WorkspaceWriteLeaseSnapshot = { ref: workspaceWriteLeaseRefFor(projectId, leaseId), revision: 1, schemaVersion: 1, lease: { schemaVersion: 1, leaseId, projectId, workspaceId, scope: P107_SCOPE_WRITER, holder: { runRef: runRef(projectId, runId), attemptRef: attRef(projectId, P107_TASK_WRITER, "att-" + runId), roleBinding: P107_ROLE_BINDING_WRITER_V1 }, grantedAt: P107_SCHEMA, expiresAt: null, status: "active", releasedAt: null, releasedBy: null, patches: [], postWriteWorkspaceRevision: null } };
  return buildWorkspaceWriteLeaseReleaseLedgerCommit(command, active, { eventId, occurredAt: P107_SCHEMA, indexSnapshot: { ref: workspaceWriteLeaseIndexRefFor(projectId, workspaceId), revision: 1, schemaVersion: 1, activeLeaseId: leaseId, activeScope: P107_SCOPE_WRITER, holderRunRef: runRef(projectId, runId) } }).events[0];
}
function patchRecordedEvents(projectId: string, workspaceId: string, leaseId: string, runId: string, patchId: string, beforeRev: number, afterRev: number, eventId: string): [import("../../src/contracts/patch.js").PatchRecordedEvent, import("../../src/contracts/workspace-lease.js").WorkspaceWriteLeaseReleasedEvent] {
  const patch: import("../../src/contracts/patch.js").PatchArtifactV1 = { schemaVersion: 1, patchId, projectId, workspaceId, goalId: P107_GOAL, taskId: P107_TASK_WRITER, planRef: p107PlanRef(projectId), taskRevision: 1, runRef: runRef(projectId, runId), attemptRef: attRef(projectId, P107_TASK_WRITER, "att-" + runId), roleBinding: P107_ROLE_BINDING_WRITER_V1, kind: "patch", title: "fix", changedPaths: ["src/p107/fix-a.ts"], bodyRef: buildP107ArtifactRef(patchId), beforeWorkspaceRevision: beforeRev, afterWorkspaceRevision: afterRev, checkResults: [{ checkId: "gate-p107", outcome: "PASS", summary: "ok" }], usedInputEvidenceRefs: [evidenceRefFor(projectId, "ev-read-a")], generatedAt: P107_SCHEMA };
  ...
  return buildPatchRecordLedgerCommit(command, { eventId, occurredAt: P107_SCHEMA, workspaceRevisionBefore: beforeRev, activeLeaseSnapshot: lease, writeIndexSnapshot: index }).events;
}
function integrationJoined(projectId: string, workspaceId: string, resultId: string, runId: string, conflicts: import("../../src/contracts/integration.js").EvidenceConflictRecordV1[], eventId: string): import("../../src/contracts/integration.js").IntegrationJoinedEvent {
  const result: IntegrationTaskResultV1 = { schemaVersion: 1, resultId, projectId, workspaceId, goalId: P107_GOAL, taskId: P107_TASK_INTEGRATION, planRef: p107PlanRef(projectId), taskRevision: 1, runRef: runRef(projectId, runId), attemptRef: attRef(projectId, P107_TASK_INTEGRATION, "att-" + runId), workspaceRevision: 1, inputs: [ { sourceTaskId: P107_TASK_READER_A, sourceRunRef: runRef(projectId, "run-a"), kind: "evidence", evidenceRef: evidenceRefFor(projectId, "ev-a"), artifactRef: null, handoffPacketRef: null }, { sourceTaskId: P107_TASK_READER_B, sourceRunRef: runRef(projectId, "run-b"), kind: "evidence", evidenceRef: evidenceRefFor(projectId, "ev-b"), artifactRef: null, handoffPacketRef: null } ], conflicts, gaps: [], explanation: conflicts.length > 0 ? "differ" : null, escalate: false, generatedAt: P107_SCHEMA };
...
const CONFLICT = (projectId: string, k: string): import("../../src/contracts/integration.js").EvidenceConflictRecordV1 => ({ conflictId: k, conflictKey: "ck-" + k, kind: "outcome_disagreement", obligationId: "obl", requirementId: "vr", planRevision: 1, workspaceRevision: 1, evidence: [ { evidenceId: "ev-a", outcome: "PASS", runRef: runRef(projectId, "run-a"), applicability: "APPLICABLE" }, { evidenceId: "ev-b", outcome: "FAIL", runRef: runRef(projectId, "run-b"), applicability: "APPLICABLE" } ], detectedAt: P107_SCHEMA });
...
describe("P1-07 SQLite projection", () => {
  ...
  it("rebuild equivalence: fresh instance full replay == incremental (JSON identical)", async () => {
    const events = [
  ...
  it("projection persists across close()/reopen and rebuild == incremental", async () => {
    const dir = tmpdir();
    ...
    try {
      ...
      const before = await rm.workspacePatches({ projectId: P107_PROJECT, workspaceId: P107_WORKSPACE });
