 ...
 * CoordinationPolicyInstalled / CoordinationPolicyActivated event stream.
 * Asserts:
 *   - the fold projects proposal / decision / policy / activation rows and a
 *     ready facts-first view (fact + decision counts, per-fact kind/revision/
 *     stale/sourceCursor/refKey lineage);
 *   - a never-advanced index -> not_found (no cursor claim);
 *   - cross-project isolation (same local ids under another project -> not_found,
 *     because proposal/decision are workspace-scoped and policy/activation are
 *     project-scoped and only the advanced project has rows);
 *   - rebuild equivalence (a fresh index replayed from the same events
 *     reproduces the view field-for-field).
 * The events are built DIRECTLY from the P1-15 human-role-collaboration fixture
...
/** Build the P1-15 event stream (proposal -> decision -> policy-install ->
 * policy-activate) as a directly-advanceable EventPage for ONE project. */
...
describe("P1-15 LANE-B InMemory projection: unifiedStatusView", () => {
  it("projects the 4 rows and returns a ready facts-first view", async () => {
    ...
    // facts: proposal + decision + policy + activation rows, in rank order.
    ...
    // Policy fact (maps to contract kind "baseline"; no dedicated policy kind).
    ...
    // Activation fact (maps to contract kind "baseline").
    ...
    // decisions: the single initial-design decision row.
    ...
    // explanations: fixed facts-first explanation for the design option.
    expect(view.explanations).toEqual([{ fact: "design option", explanation: "由 HumanCollaboration 解释", stale: false }]);

    // freshness = observed cursor after the 4-event fold.
