/**
 * P1-07 Control admission tests: ConfiguredWorkspaceCapabilityPolicy + the pure
 * evaluateWorkspaceOperation / scopeWithinCapability rules.
 *
 * Capability = explicitly configured execution support ∩ declared
...
import { evaluateWorkspaceOperation, scopeWithinCapability } from "../../src/control/control-engine/policies/workspace-operation.js";
import type { WorkspaceCapabilitiesV1 } from "../../src/contracts/workspace-capability.js";
...
describe("ConfiguredWorkspaceCapabilityPolicy", () => {
  it("explicit support enables read+write with no max scope", async () => {
    const adapter = new ConfiguredWorkspaceCapabilityPolicy({ workspaceRead: true, workspaceWrite: true, maxWriteScope: null });
  ...
  it("tools include 'write' but support matrix closes write -> workspaceWrite=false", async () => {
    const adapter = new ConfiguredWorkspaceCapabilityPolicy({ workspaceRead: true, workspaceWrite: false, maxWriteScope: null });
    const result = await adapter.capabilitiesFor(envelope(P107_DECLARED_WRITE_PERMISSIONS_V1));
    expect(result.status).toBe("ready");
    if (result.status === "ready") expect(result.capabilities.workspaceWrite).toBe(false);
  });

  it("explicit null support -> { status: 'unsupported' } (no silent degrade)", async () => {
    const adapter = new ConfiguredWorkspaceCapabilityPolicy(null);
    const result = await adapter.capabilitiesFor(envelope(P107_DECLARED_WRITE_PERMISSIONS_V1));
    expect(result).toEqual({ status: "unsupported" });
  });

  it("maxWriteScope is carried through to the capabilities", async () => {
    const support: WorkspaceCapabilitySupport = { workspaceRead: true, workspaceWrite: true, maxWriteScope: P107_SCOPE_WRITER };
    const adapter = new ConfiguredWorkspaceCapabilityPolicy(support);
    const result = await adapter.capabilitiesFor(envelope(P107_DECLARED_WRITE_PERMISSIONS_V1));
    expect(result.status).toBe("ready");
  ...
  it("capabilitiesFor is async and does not throw on missing writeScope", async () => {
    const adapter = new ConfiguredWorkspaceCapabilityPolicy({ workspaceRead: true, workspaceWrite: true, maxWriteScope: null });
    const response = adapter.capabilitiesFor(envelope({ tools: ["read"], writeScope: [] }));
    expect(response).toBeInstanceOf(Promise);
    const result = await response;
    expect(result.status).toBe("ready");
  });

  it("declared read permission cannot enable unsupported read access", async () => {
    const policy = new ConfiguredWorkspaceCapabilityPolicy({ workspaceRead: false, workspaceWrite: true, maxWriteScope: null });
    expect(await policy.capabilitiesFor(envelope(P107_DECLARED_WRITE_PERMISSIONS_V1))).toMatchObject({
      status: "ready", capabilities: { workspaceRead: false, workspaceWrite: true },
    });
  });

  it("caller configuration mutations cannot change configured support or its scope cap", async () => {
    const support: WorkspaceCapabilitySupport = { workspaceRead: true, workspaceWrite: false, maxWriteScope: { ...P107_SCOPE_WRITER } };
    const policy = new ConfiguredWorkspaceCapabilityPolicy(support);
    support.workspaceRead = false;
    support.workspaceWrite = true;
    support.maxWriteScope!.id = "src/elsewhere";
    support.maxWriteScope!.projectId = "another-project";
    expect(await policy.capabilitiesFor(envelope(P107_DECLARED_WRITE_PERMISSIONS_V1))).toMatchObject({
      status: "ready", capabilities: { workspaceRead: true, workspaceWrite: false, maxWriteScope: P107_SCOPE_WRITER },
    });
  });

  it("a returned capability and scope cap cannot mutate later responses", async () => {
    const policy = new ConfiguredWorkspaceCapabilityPolicy({ workspaceRead: true, workspaceWrite: true, maxWriteScope: P107_SCOPE_WRITER });
    const first = await policy.capabilitiesFor(envelope(P107_DECLARED_WRITE_PERMISSIONS_V1));
    expect(first.status).toBe("ready");
    if (first.status !== "ready") throw Error("expected ready capability");
    first.capabilities.workspaceWrite = false;
    first.capabilities.maxWriteScope!.id = "src/elsewhere";
    expect(await policy.capabilitiesFor(envelope(P107_DECLARED_WRITE_PERMISSIONS_V1))).toMatchObject({
      status: "ready", capabilities: { workspaceWrite: true, maxWriteScope: P107_SCOPE_WRITER },
    });
  });
});
