...
export async function runP110ControlScenario(h: P1_10HarnessLike): Promise<P110ControlScenarioResult> {
  ...
  const runRef = p110RunRef(world.projectA.workRun.runId);

  const pause = buildP110Intent({ intentId: p110IntentId("pause", 1), kind: "pause", scope: { projectId: P108_PROJECT_A, workspaceId: P110_WORKSPACE, goalId: world.previews.find((p) => p.projectId === P108_PROJECT_A)!.goalId, taskId: P108_TASK_WORK, runRef } });
  ...
  const resume = buildP110Intent({ intentId: p110IntentId("resume", 2), kind: "resume", scope: { projectId: P108_PROJECT_A, workspaceId: P110_WORKSPACE, goalId: world.previews.find((p) => p.projectId === P108_PROJECT_A)!.goalId, taskId: P108_TASK_WORK, runRef } });
  ...
  const steer = buildP110Intent({ intentId: p110IntentId("steer", 3), kind: "steer", scope: { projectId: P108_PROJECT_A, workspaceId: P110_WORKSPACE, goalId: world.previews.find((p) => p.projectId === P108_PROJECT_A)!.goalId, taskId: P108_TASK_WORK, runRef }, steer: { directive: "先补一个复现用例再继续实现", payloadDigest: "sha256:fixture", safePointOnly: true, expectedRunRef: runRef } });
  ...
  const cancel = buildP110Intent({ intentId: p110IntentId("cancel", 4), kind: "cancel", scope: { projectId: P108_PROJECT_A, workspaceId: P110_WORKSPACE, goalId: world.previews.find((p) => p.projectId === P108_PROJECT_A)!.goalId, taskId: P108_TASK_WORK, runRef } });
