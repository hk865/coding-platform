...
export type P114RestartEvidence = {
...
export async function runP114RestartScenario(h: PersistentSqliteHarness): Promise<P114RestartEvidence> {
  ...
  const dec = await h.ledger.load(p114DecisionRef(P114_PROJECT));
  expect(dec.status).toBe("found");
  const gate = await h.ledger.load(p114GateRef(P114_PROJECT));
  expect(gate.status).toBe("found");
  const act = await h.ledger.load(p114ActivationRef(P114_PROJECT));
  expect(act.status).toBe("found");
  return {
    candidateDigest: scen.candidate.contentDigest,
    decisionOutcome: scen.decision.outcome,
    gateStatus: scen.gate.status,
    activationFrom: JSON.stringify(scen.activation.fromPin),
  };
}
...
export async function verifyP114AfterRestart(restarted: PersistentSqliteHarness, evidence: P114RestartEvidence): Promise<void> {
  ...
  const act = await restarted.ledger.load(p114ActivationRef(P114_PROJECT));
  expect(act.status).toBe("found");
  if (act.status === "found") expect(JSON.stringify((act.snapshot as { activation: { fromPin: unknown } }).activation.fromPin)).toBe(evidence.activationFrom);
}
