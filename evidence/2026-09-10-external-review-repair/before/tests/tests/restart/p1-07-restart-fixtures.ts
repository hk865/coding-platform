...
export async function runP107RestartScenario(h: PersistentSqliteHarness): Promise<P107RestartEvidence> {
  ...
  const leaseView = await th.workspaceLeaseView({ projectId: before.readerARun.projectId, workspaceId: P107_WORKSPACE });
  const patchView = await th.workspacePatches({ projectId: before.readerARun.projectId, workspaceId: P107_WORKSPACE });
...
export async function verifyP107AfterRestart(restarted: PersistentSqliteHarness, evidence: P107RestartEvidence): Promise<void> {
  ...
  const ws = await restarted.ledger.load({ aggregateType: "Workspace", projectId: evidence.before.readerARun.projectId, workspaceId: P107_WORKSPACE });
  ...
  const leaseView = await th.workspaceLeaseView({ projectId: evidence.before.readerARun.projectId, workspaceId: P107_WORKSPACE });
  expect(JSON.stringify(leaseView)).toBe(evidence.leaseViewBefore);
  const patchView = await th.workspacePatches({ projectId: evidence.before.readerARun.projectId, workspaceId: P107_WORKSPACE });
