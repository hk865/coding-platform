...
**用户已明确本轮范围：本轮已完成独立Reviewer，现将架构与Reviewer交其他模型审查；其他核心功能待审查后动工。** VR-01最终全量日志已独立确认并验收，现已完成[VR-02](../dev_docs/planning/active/core-verification/VR-02.md)。架构重构批次完成与完整自治产品完成分开判断。上次安全退出与源码身份见[暂停检查点](../dev_docs/planning/active/core-verification/PAUSED.md)。全部未提交修改和历史资料保留。
...
2026-09-09 全模块核心义务补齐正在实施：已新增材料撤销（原授权不可变、撤销事件、canonical 读取复核）及项目级 TS/JS 索引，局部证据见 [施工与覆盖记录](../dev_docs/verification/2026-09-09-module-completion.md)。同工作区跨 Goal 精确历史读取与 Python 语义服务已通过局部复验；用户已允许显式跨 Workspace 授权并禁止自动共享，协议测试已覆盖两套 Adapter；Context 选材/容量与检查中间日志、Evidence/对账入口已实现部分消费者，真实 TS/JS 架构 sourceBinding/delta 通过一次 SQLite 链路。阶段 A–C 仍有核心缺口，D–G 及真实项目总验收未完成；前述日期条目保留历史验证边界。
...
flowchart TB
  subgraph DataPlane["Data Plane"]
    ...
    Workspace["WorkspaceReader · 重大缺口<br/>TS/JS 显式源码映射图部分接通<br/>TS/JS、Python、C++ 有局部语义查询<br/>探索消费者已接完整来源 pin，其余角色欠缺"]
  ...
  class Select,Workspace,Refill gap;
```
...
flowchart TB
  ...
  subgraph ControlPlane["Control Plane"]
    ...
    subgraph VE["VerificationEngine · 重大缺口"]
    ...
    Arch["ArchitectureReconciler · 部分实现<br/>精确 sourceBinding／真实差分与来源问题<br/>基线演进 UI／真实角色消费未闭环"]
  ...
  Source["WorkspaceReader · 重大缺口"] -.-> Arch
  Arch --> Receipt
  Receipt --> Control
  classDef done fill:#dcfce7,stroke:#15803d,color:#14532d;
  classDef partial fill:#fef3c7,stroke:#b45309,color:#78350f;
  ...
  class Arch,Source gap;
  class Roles,Rework missing;
  style PC fill:#fff5f5,stroke:#b91c1c,color:#7f1d1d
  style VE fill:#fff5f5,stroke:#b91c1c,color:#7f1d1d
```

Control 的归约是确定性代码，不能用模型“完成”替代验收。PlanCompiler 和 VerificationEngine 的局部实现存在，但缺少关键真实角色消费者，因此整体标红。Reconciler 已去掉固定业务结论，真实差分与来源绑定已有一次 SQLite 证据；基线演进和角色消费仍未闭环。
...
flowchart TB
  ...
  class Plan,Query gap;
```

“显式规则能随任务材料被消费”与“用户能在设置中持久管理规则并影响后续任务”是两个功能；前者已有，后者未接通。真实模式中栏提问／发送目前禁用，独立语义查询仍注入固定回答 Adapter。图形展示后端事实，并不能补出后端不存在的角色分工或代码关系。绿色 UI 节点只表示对应基础接线，未宣称 Windows 原生部署或全部浏览器交互验收通过。
...
flowchart TB
  ...
  Snapshot -.-> Runtime
  Plan["PlanCompiler／VerificationEngine · 重大缺口"] -.-> RoleRuns
  RoleRuns -.-> Runtime
  classDef done fill:#dcfce7,stroke:#15803d,color:#14532d;
  classDef partial fill:#fef3c7,stroke:#b45309,color:#78350f;
  classDef missing fill:#e5e7eb,stroke:#6b7280,color:#374151,stroke-dasharray:5 3;
...
flowchart LR
  ...
  ArchitectureReconciler["ArchitectureReconciler<br/>部分：真实差分；演进消费未闭环"]
  ...
  class PlanCompiler,VerificationEngine,ArchitectureReconciler,WorkspaceReader gap;
```

此前工程审计发现的 PlanCompiler 直接读 Ledger、ArchitectureReconciler 直接读 WorkspaceReader、原生来源实现混在 runtime 等结构漂移，已在本次架构批次中归位并独立核对。当前图与 ARCHITECTURE 的 34 条 Module 依赖一致（2026-09-10 修正：原声明的 PlanCompiler → ArtifactVault 边没有实现消费者，已从三处一致删除，不是新增实现去凑边数）；实际 DI 与源码证据见[架构验收](../dev_docs/verification/2026-09-09-architecture-rebuild/acceptance.md)。下表保留的是当前接线及功能缺口，不能把旧漂移描述继续当作现状。
...
| VerificationEngine → Control / Context / Vault | 工具轮次与独立Reviewer生命周期归本Module；Context取材并复核，Control接纳与归约。Reviewer已按本票验收；返工待外部审查后动工 |
...
| --- | --- | --- | --- | --- |
| HumanCollaboration（交互入口） | 部分可用 | 创建目标、查投影、转交变更；探索/历史材料按接口处理；工作台消费真实工具轮次、Reviewer状态、原报告与请求回执 | 完整协商、角色反馈、记忆/角色设置未接 | Interface 扩展＋实际消费者 |
...
| ControlEngine（确定性控制） | 基础路径可用，自治欠缺 | 命令守卫与确定性归约；独立审阅协议、Work/Result及分组Evidence原子接纳，版本／权限／当前资格与CAS在本模块复核 | 自治修订授权与消费缺失 | 恢复事实契约＋内部策略 |
| DispatchEngine（派发与运行事实） | 普通/初始计划已接，Reviewer已按本票验收 | 持久outbox、canonical readiness、Run事实；Reviewer复用原事实消费和只读运行，授权/租约/恢复在Module内 | 修订与真实进程强杀恢复未闭环 | 内部驱动与实际恢复协议 |
| VerificationEngine（验证编排） | 工具轮次已验收，独立Reviewer已按本票验收 | 原工具报告、不可变配置/来源、逐VR检查与Reviewer报告资格、Evidence及恢复归Module；HTTP/UI只消费接口 | Reviewer全量与独立审计已通过；返工与OS强杀另待；stale不自动撤销旧Evidence | 架构与Reviewer已交付审查材料，其他核心暂不动工 |
| ArchitectureReconciler（架构对账） | sourceBinding/实际差分部分接通 | 不可变基线绑定源码 manifest/commit/映射；真实 TS/JS 差分、未知关系与来源问题持久登记，回执拒绝停止 | 初始基线准备/演进 UI、全角色/修复决定消费仍缺 | 继续正式消费者与恢复接线 |
...
| ReadModelIndex（查询投影） | 投影可用，完成筛选已修 | 两套视图共享Reviewer记录与当前资格解释，保留最后已提交归约及原租约；投影重建一致，不提交业务命令 | 协调／查询结束资格尚缺；协议采用checkpoint期间phase可能滞后，不能代替当前资格 | 补结束事实消费者，保留模块内重复投影维护债 |
| ContextCompiler（上下文选材） | 选材/来源绑定已归位，角色覆盖部分 | 探索、协调、历史、源码图及验证材料由明确端口提供；Reviewer固定配置/预算、有界包与完整grant复核，当前接纳和历史报告读取分开 | 模型对应 tokenizer、全部真实角色与相关历史自动继承尚缺 | 内部算法＋角色材料契约 |
...
已有 [Query 驱动](/mnt/d/1.project/Software/agent_platform/src/control/query-drive.ts) 实现持久任务／运行状态与调用接口；应用真实查询已接 [只读内核运行时](/mnt/d/1.project/Software/agent_platform/src/runtime/read-only-query-runtime.ts)，固定回答 Adapter 保留为显式样例路径。`tests/app/semantic-query.test.ts` 验证实际内核搭配模型 stub 的来源读取、禁止写入及重开。[持久宿主](/mnt/d/1.project/Software/agent_platform/src/harness/persistent-harness.ts) 的公开 snapshot 默认返回 unsupported；[真实执行器](/mnt/d/1.project/Software/agent_platform/src/runtime/coding-agent-runtime.ts) 声明 supportsSnapshot=false。因此仍不能说已经具备完整的“向执行者索取并交回决策材料”。缺口主要在公开材料能力、按角色路由和持久反馈，而不只是按钮。
...
4. **持久化与真实接续。** 通用 Vault 正文重启丢失已修；仍需补真实运行的暂停／继续／取消／换手适配与能力声明一致性。对未知副作用先对账；不能靠重跑冒充恢复。验收要在新进程／新 Agent 中保留义务、关键理由、失败和未解项。
...
架构与 Reviewer 批次（[VR-01](../dev_docs/planning/active/core-verification/VR-01.md)、[VR-02](../dev_docs/planning/active/core-verification/VR-02.md)）已验收；2026-09-10 外部独立审查发现的问题正由[外部审查修复工作包](../dev_docs/planning/active/external-review-repair/README.md)收敛，其余核心功能待用户审查后动工，顺序见[核心功能接续](../dev_docs/verification/2026-09-09-architecture-rebuild/core-continuation.md)。此前[模块职责修复](../dev_docs/verification/2026-09-09-architecture-repair.md) 保留原始证据。下表是原覆盖清单，不覆盖本批优先级，也不表示已完成。代码入口见 [模块 README 索引](../../../../agent_platform/src/README.md)。
...
| 5 | ArchitectureReconciler | 已补 sourceBinding 与真实 delta/Finding；显式 dependencyRules 正在复验，拒绝回执停止；仍需基线演进 UI、真实角色与恢复闭环 |
