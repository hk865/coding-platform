...
it('filters pending Reviewer work before applying the ordinary-dispatch fetch window', async () => {
  ...
  expect((await ledger.pendingDispatchIntents(1, { workKind: 'ordinary' }))[0]!.intent.taskId).toBe('task-verify-view');
  expect((await ledger.pendingDispatchIntents(1, { workKind: 'review' }))[0]!.intent.work?.kind).toBe('review');
  let selected = 0;
  ...
  for (let i = 0; i < 3; i++) expect(await dispatch.drive({ reason: 'audit', maxIntents: 1 })).toMatchObject({ scanned: 1, started: 0 });
