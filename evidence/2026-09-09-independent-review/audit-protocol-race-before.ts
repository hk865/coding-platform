...
it('records the corresponding Task reduction window but confirms the downstream live-plan guard', async () => {
  ...
  expect(receipt).toMatchObject({ status: 'committed', phase: 'satisfied' });
  expect((await loadLivePlan(ledger, h.plan)).tasks.find(t => t.taskId === scope.taskId)?.phase).toBe('blocked');
});

it('records the narrow legacy protocol-adoption versus Goal reduction commit interleaving', async () => {
  ...
  expect(adopted).toBe(true);
  expect(receipt).toMatchObject({ status: 'committed', phase: 'COMPLETED' });
  ...
  expect(status).toMatchObject({ status: 'ready', goal: { phase: 'COMPLETED' } });
});
