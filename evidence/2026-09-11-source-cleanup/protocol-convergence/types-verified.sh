...
case "$1" in
 types) run backend-types-verified node .local/linux-test-tools/node_modules/typescript/bin/tsc --noEmit && run ui-types-verified node src/ui/node_modules/typescript/bin/tsc --noEmit -p src/ui/tsconfig.json && run boundaries-verified node scripts/check-module-boundaries.mjs --output="$evidence/boundaries.json" ;;
 targeted) run targeted bash scripts/test-wsl.sh tests/contracts tests/context/planning-context-compiler.test.ts tests/control/goal-change.test.ts tests/control/planning-interface.test.ts tests/control/rework-plan-compiler.test.ts tests/control/autonomous-rework.test.ts tests/control/rework-drive.test.ts tests/verification/verification-engine.test.ts tests/verification/rework-issues.test.ts tests/app/semantic-loop.test.ts tests/runtime/context-continuation-adapter.test.ts ;;
