...
case "$1" in
 types) run backend-types-final node .local/linux-test-tools/node_modules/typescript/bin/tsc --noEmit && run ui-types-final node src/ui/node_modules/typescript/bin/tsc --noEmit -p src/ui/tsconfig.json && run boundaries-final node scripts/check-module-boundaries.mjs --output="$evidence/boundaries.json" ;;
