...
evidence=evidence/2026-09-11-source-cleanup/export-pruning
...
case "$1" in
 ...
 targeted) run targeted bash scripts/test-wsl.sh tests/contracts tests/control/planning-interface.test.ts tests/control/rework-plan-compiler.test.ts tests/control/autonomous-rework.test.ts tests/control/rework-drive.test.ts tests/verification/verification-engine.test.ts tests/verification/rework-issues.test.ts tests/app/semantic-loop.test.ts tests/runtime/context-continuation-adapter.test.ts ;;
 full) run full-tests bash scripts/test-wsl.sh ;;
 build) run build pnpm build && run ui-artifacts node scripts/verify-ui-build.mjs ;;
 browser)
  export FIXTURE_DIR="$PWD/.local/export-pruning-browser-20260911/root"
  export FIXTURE_DATA="$PWD/.local/export-pruning-browser-20260911/data"
  [[ ! -e "$PWD/.local/export-pruning-browser-20260911" ]] || exit 2
