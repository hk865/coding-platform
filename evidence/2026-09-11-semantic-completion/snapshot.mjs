...
function scan(root,dir=root){return fs.readdirSync(dir,{withFileTypes:true}).flatMap(e=>{
 ...
 if(skip.has(e.name)||rel==='src/app/public/workbench'||rel==='dev_docs/verification/2026-09-11-semantic-completion')return [];
...
fs.writeFileSync(path.join(product,'evidence/2026-09-11-semantic-completion',process.argv[2]||'baseline.json'),JSON.stringify(result,null,2));
