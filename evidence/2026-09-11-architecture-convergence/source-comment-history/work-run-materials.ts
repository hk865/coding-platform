/**
 * RW-12 ContextCompiler — 派发时把「工作身份 + 相关历史材料 + 角色必读材料」编译进既有 ContextBundle。
 *
 * 权威：ADR 0003 D4（历史材料在**派发时**编译进既有 ContextBundle，选材理由与缺口写入 manifest；
 * 取材与组装属 ContextCompiler，不新增 Module）、ARCHITECTURE「Context 生命周期与编排」与
 * module-boundaries「角色规格与真实 Context（D4）」、human/context-management.md「材料由谁管理」、
 * 核心义务映射义务 #4/#5。
 * 施工历史（RW-15 暴露的 fail-closed、RW-17 四类真实通道怎么接上、以及为什么「code」走内核工作区
 * 读取而不是 WorkspaceReadPort 的图读取）见本 Module README 的「角色必读材料」一节；本文件只留
 * 判据、不变量与决定链接。
 *
 * ── 这个编译器什么时候跑（不新增运行期读取路径）───────────────────────────────
 * 它**不**被 WorkerRuntime 在运行中调用：在派发准备阶段（模型调用之前）运行一次，产物是既有
 * `RuntimeContextMaterials`，随后沿既有 `TaskEnvelope.bundleRef → assembleRuntimeContext` 通道
 * 进入 manifest.selected（`selectedBecause` + `sourceRefs`）。因此运行期没有第二次选材、第二次
 * 权限复核，也没有"边跑边拉材料"的旁路；选材发生在**权限与版本都还是 claim 时的那一份**的时候。
 *
 * ── 不变量（是行为，不只是注释）───────────────────────────────────────────────
 *  1. 材料只是**参考**：不授予工具、写入范围或租约，不参与完成归约；本编译器只读 ledger／vault，
 *     只提交 WorkContextPort 自己的 body-first 有界包，不写任何 Task／Goal／Evidence 事实；
 *  2. 资格在类型层封死：历史材料只能是 `historical_explanation`，源码等参考材料只能是 `reference`，
 *     不存在把它们表达成当前事实的取值；
 *  3. 材料不携带授权：Run 的权限永远只来自 `TaskEnvelope.permissions`；每条材料的来源与版本随
 *     `sourceRefs` 进入 manifest，来源或版本不成立时该条报缺口而不是被"尽量补全"；
 *  4. **缺失 ≠ 确定为空**：通道不存在／宿主未接线／读不到／越权／工作区版本已前进／索引与聚合不一致
 *     → `needs_material`（模型调用之前终止这次运行）；"确定为空"（首次运行、聚合从未创建）是
 *     **可核对的事实**，随材料与 gaps 写明依据。当成缺失会让第一次运行永远无法开始，当成已满足才是伪造；
 *  5. **有界**：上限造成的未选入一律写进 `gaps`（写明哪一类、为什么、上限是多少），绝不静默裁剪；
 *  6. 桥表 `ROLE_MATERIAL_CHANNEL_V1` 是 `Record<RoleMaterialKindV1, ...>` **全键穷尽**：新增一类
 *     必读材料而桥表少一个键，`npx tsc --noEmit -p tsconfig.json` 直接失败。
 *
 * ── 归属（RC-02）────────────────────────────────────────────────────────────
 * 「code」类的**取材判据**在本文件与 [role-source-index.ts](role-source-index.ts)（只消费窄端口）；
 * 工作区的列举、读取与路径边界适配属 WorkspaceReader（`data/workspace-reader/role-source-reader.ts`），
 * 拒绝前缀只有 `data/workspace-reader/denied-prefixes.ts` 一个来源。
 */
...
/** 本编译器能供应的材料类别。RW-11 的角色规格 requiredMaterials 映射到这些键；
 *  RW-17 之后 contract／code／evidence／decision 四类各有真实通道（不再是"本入口不拥有"）。 */
export type WorkRunMaterialKind = 'work-identity' | 'work-notes' | 'history' | 'contract' | 'code' | 'evidence' | 'decision';

/**
 * 两套词表的语义差别（RW-15 / L1）——它们**不是**同义词，桥接必须逐键显式表态：
 *   - `RoleMaterialKindV1`（contracts/role-spec.ts）是**角色要求的类别**：规格作者视角，
 *     说明"这次工作必须读到什么"（contract／code／evidence／history／decision）；
 *   - `WorkRunMaterialKind` 是**本入口能供应的通道**：派发入口视角，说明"它自己取得到什么"
 *     （work-identity／work-notes／history）。
 * 一个类别在角色侧存在，不代表本入口有对应通道；因此桥表的值有两义：
 *   - 通道名：本入口在该通道上供应这一类别（缺失 → needs_material，既有行为）；
 *   - null   ：本入口**没有**该通道（见 compile()：解析到规格即 fail-closed，绝不冒充已满足）。
 * `suppliedBy` 是给人看的：写清这一类别今天由哪条通道供应／应当由哪条通道供应。
 *
 * `Record<RoleMaterialKindV1, ...>` 是**全键穷尽**的：`ROLE_MATERIAL_KINDS` 新增一类必读材料时，
 * 这里少一个键就编译失败（`npx tsc --noEmit -p tsconfig.json`），强制新增类别必须显式表态。
 * 验证方法：在 contracts/role-spec.ts 的 ROLE_MATERIAL_KINDS 里临时加一个成员，运行上面的
 * 类型检查即报 "Property '<新成员>' is missing"；删掉该成员即恢复。
 */
...
/** RW-17：角色必读材料 categories 的取材上限（超限如实进 gaps，不静默裁剪）。 */
...
export type WorkRunMaterialCompilerDeps = {
  ...
  /**
   * RW-17：「code」类必读材料的宿主侧能力——受权限约束、按工作区版本读取的有界源码索引。
...
export class WorkRunMaterialCompiler {
  ...
  async compile(request: WorkRunMaterialRequest): Promise<WorkRunMaterialResult> {
    ...
    const materials: RuntimeContextMaterials = {
      ...
      /** RW-17：`rules` 承载「contract」类必读材料（本 Run 的已接受任务包与计划固定 pin，见
       *  ROLE_MATERIAL_CHANNEL_V1）。前驱报告材料仍由既有通道各自供应，本编译器不重复它们。 */
      rules: [...collected.rules],
      predecessors: [],
      /** RW-17：「evidence」类沿用既有 evidenceRefs 语义——本任务与直接前驱的已接纳证据**索引**。 */
  ...
  private async resolveRoleSpec(request: WorkRunMaterialRequest): Promise<
    ...
    | {
        ...
        /** RW-17：可选材料里本入口拥有的通道（按规格顺序、去重）。它们照常取材，但缺失只是缺口。 */
  ...
  > {
    ...
    const record: RuntimeRoleSpecMaterials = {
      ...
      /**
       * RW-18：这是角色的**声明性产出期望**（供 Agent 与读者参考），随既有通道进入 ContextBundle；
  ...
  private async selectHistory(
    ...
    requestedByRunRef: runRef,
    /** RW-18：如实回显本 Run 实际持有的权限。它是审计信息，**不是**历史访问的准入判据。 */
    declaredPermissions: { tools: [...request.permissions.tools], writeScope: [...request.permissions.writeScope] },
